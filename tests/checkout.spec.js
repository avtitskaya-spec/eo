...
test.describe('Добавление товара в корзину', () => {
    test.beforeEach(async ({page}) => {
        ...
        test('Отображения добавленного товара в мини-корзине', async ({page, app}) => {
            await page.waitForTimeout(10000);
            await app.product.addProductToCart();
            await expect(app.miniCart.cartCountBadge).toHaveCount(1);
        });

        test('Открытие попапа "Все цвета"', async ({page, app}) => {
        ...
        test('Добавление в избранное: кнопка меняет состояние', async ({page, app}) => {
